...
import { useCreateUserMutation, useUpdateUserMutation, useGetUserByIdQuery } from '../reducers/appslice';
...
const UserForm = () => {
  ...
  const { data: userData, isLoading } = useGetUserByIdQuery(id); // Fetch user data by ID
  ...
  const setInitialValues = (userData) => {
          ...
          {({ isSubmitting }) => (
            <Form className="space-y-6">
              ...
              <div>
                ...
                <Field id="profilePicture" name="profilePicture" type="file" className="w-full border text-green-500 border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:border-blue-500" />
              ...
              {/* Submit Button */}
              <div>
                <button type="submit" disabled={isSubmitting || isLoading} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 rounded-md focus:outline-none focus:ring focus:ring-blue-200 transition duration-300">
